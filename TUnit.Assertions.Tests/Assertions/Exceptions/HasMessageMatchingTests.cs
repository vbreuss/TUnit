...
public class HasMessageMatchingTests
{
        ...
        string message, string pattern, bool expectMatch)
    {
        ...
        var sut = async ()
            => await Assert.That(action).ThrowsException().Which.HasMessageMatching(pattern);
    ...
    public async Task Fails_For_Different_Messages()
    {
        ...
        var expectedMessage = """
                              ...
                              at Assert.That(action).ThrowsException().Which.HasMessageMatching(message2)
                              """;
        Exception exception = new(message1);
        Action action = () => throw exception;

        var sut = async ()
            => await Assert.That(action).ThrowsException().Which.HasMessageMatching(message2);
    ...
    public async Task Returns_Exception_When_Awaited()
    {
        ...
        var result = await Assert.That(action).ThrowsException().Which.HasMessageMatching(matchingMessage);
    ...
    public async Task Succeeds_For_Matching_Message()
    {
        ...
        var sut = async ()
            => await Assert.That(action).ThrowsException().Which.HasMessageMatching(matchingMessage);
        ...
        string message, string pattern, bool expectMatch)
    {
        var expectedExpression =
            "*Assert.That(action).ThrowsException().Which.HasMessageMatching(StringMatcher.AsWildcard(pattern).*";
        Exception exception = new(message);
        Action action = () => throw exception;

        var sut = async ()
            => await Assert.That(action).ThrowsException()
                .Which.HasMessageMatching(StringMatcher.AsWildcard(pattern).IgnoringCase());
    ...
    {
        var expectedExpression =
            "*Assert.That(action).ThrowsException().Which.HasMessageMatching(StringMatcher.AsRegex(pattern))*";
        Exception exception = new(message);
        Action action = () => throw exception;

        var sut = async ()
            => await Assert.That(action).ThrowsException()
                .Which.HasMessageMatching(StringMatcher.AsRegex(pattern));
    ...
    {
        var expectedExpression =
            "*Assert.That(action).ThrowsException().Which.HasMessageMatching(StringMatcher.AsRegex(pattern).Ign*";
        Exception exception = new(message);
        Action action = () => throw exception;

        var sut = async ()
            => await Assert.That(action).ThrowsException()
                .Which.HasMessageMatching(StringMatcher.AsRegex(pattern).IgnoringCase());
